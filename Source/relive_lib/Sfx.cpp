...
s32 relive::SFX_Play_Stereo(relive::SoundEffects sfxId, s32 leftVol, s32 rightVol, FP scale)
{
    if (scale == FP_FromDouble(0.5))
    {
        leftVol = 2 * leftVol / 3;
...
s32 relive::SFX_Play_Camera(relive::SoundEffects sfxId, s16 volume, CameraPos direction, FP scale)
{
    if (!volume)
    {
        volume = relive::GetSfx(sfxId).mDefaultVolume;
