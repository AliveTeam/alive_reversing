    ...
    : BaseAliveGameObject(9)
{
    ...
    mAnim.field_1C_fn_ptr_array = kFlyingSlig_Anim_Frames_Fns_55EFC4;
    ...
    mAnim.mAnimFlags.Set(AnimFlags::eBit5_FlipX, field_118_data.field_10_data.field_A_direction == XDirection_short::eLeft_0);
...
s32 FlyingSlig::CreateFromSaveState(const u8* pBuffer)
{
    ...
    if (pFlyingSlig)
    {
        pFlyingSlig->mPathTLV = nullptr;
        pFlyingSlig->mCollisionLine = nullptr;
        ...
        pFlyingSlig->mPathNumber = pSaveState->field_14_path_number;
        pFlyingSlig->mLvlNumber = pSaveState->field_16_lvl_number;
        pFlyingSlig->mSpriteScale = pSaveState->field_18_sprite_scale;

        pFlyingSlig->field_27C_r = pSaveState->field_1C_oldr;
        pFlyingSlig->mRed = pSaveState->field_1C_oldr;

        pFlyingSlig->field_27E_g = pSaveState->field_1E_oldg;
        pFlyingSlig->mGreen = pSaveState->field_1E_oldg;

        pFlyingSlig->field_280_b = pSaveState->field_20_oldb;
        pFlyingSlig->mBlue = pSaveState->field_20_oldb;
        ...
        pFlyingSlig->mAnim.Set_Animation_Data(animRec.mFrameTableOffset, ppRes);

        pFlyingSlig->mAnim.field_92_current_frame = pSaveState->field_26_current_frame;

        pFlyingSlig->mAnim.mFrameChangeCounter = pSaveState->field_28_frame_change_counter;

        pFlyingSlig->mAnim.mAnimFlags.Set(AnimFlags::eBit3_Render, pSaveState->field_2A_bAnimRender & 1);
        pFlyingSlig->mAnim.mAnimFlags.Set(AnimFlags::eBit5_FlipX, pSaveState->field_22_bAnimFlipX & 1);
        pFlyingSlig->mGameObjectFlags.Set(BaseGameObject::eDrawable_Bit4, pSaveState->field_2B_bDrawable & 1);

        if (IsLastFrame(&pFlyingSlig->mAnim))
        {
            pFlyingSlig->mAnim.mAnimFlags.Set(AnimFlags::eBit18_IsLastFrame);
        }
        ...
        pFlyingSlig->mLastLineYPos = FP_FromInteger(pSaveState->field_34_lastLineYPos);
        pFlyingSlig->mAliveGameObjectFlags.Set(Flags_114::e114_Bit9_RestoredFromQuickSave);
        pFlyingSlig->mCollisionLineType = -1;

        if (pSaveState->field_36_line_idx != -1)
        {
            pFlyingSlig->mCollisionLineType = pSaveState->field_36_line_idx;
        }
...
s32 FlyingSlig::VGetSaveState(u8* pSaveBuffer)
{
    ...
    pState->field_1C_oldr = mRed;
    pState->field_1E_oldg = mGreen;
    pState->field_20_oldb = mBlue;

    pState->field_22_bAnimFlipX = mAnim.mAnimFlags.Get(AnimFlags::eBit5_FlipX);
    pState->field_24_current_state = mCurrentMotion;
    pState->field_26_current_frame = mAnim.field_92_current_frame;
    ...
    if (mCollisionLine)
    {
        pState->field_36_line_idx = static_cast<s16>(mCollisionLine - sCollisions_DArray_5C1128->field_0_pArray);
    }
...
{
    if (gMap.mCurrentLevel != gMap.mLevel || gMap.mOverlayId != gMap.GetOverlayId() || (gMap.mCurrentPath != gMap.mPath && (this != sControlledCharacter || field_17E_flags.Get(Flags_17E::eBit13_Persistant))))
    {
    ...
    if (mAliveGameObjectFlags.Get(Flags_114::e114_Bit9_RestoredFromQuickSave))
    {
        ...
        if (!IsPossessed())
        {
            if (mCollisionLineType != -1)
            {
                ...
                mCollisionLine = sCollisions_DArray_5C1128->Get_Line_At_Idx(mCollisionLineType);
                sub_4348A0();
...
void FlyingSlig::Movement()
{
    if (!IsPossessed())
    {
        ...
        mVelX = posXY.field_0_x - mXPos;
        mVelY = posXY.field_4_y - mYPos;
    }
    else
    {
    ...
    if (field_17E_flags.Get(Flags_17E::eBit5_Throw))
    {
        if (static_cast<s32>(sGnFrame_5C1B84) > field_150_grenade_delay && 
            (mAliveGameObjectFlags.Get(Flags_114::e114_Bit4_bPossesed) || SwitchStates_Get(field_17C_launch_switch_id) ||
             mAliveGameObjectFlags.Get(Flags_114::e114_Bit4_bPossesed)) && CanThrowGrenade_43A490())
        {
            ThrowGrenade_43A1E0();
        }
    }
    ...
    s32 v37 = {};
    if (Math_Distance_496EB0(0, 0, FP_GetExponent(mVelX), FP_GetExponent(mVelY)) >= 0)
    {
        v37 = Math_Distance_496EB0(0, 0, FP_GetExponent(mVelX), FP_GetExponent(mVelY));
    }
    else
    {
        v37 = -Math_Distance_496EB0(0, 0, FP_GetExponent(mVelX), FP_GetExponent(mVelY));
...
s16 FlyingSlig::VTakeDamage(BaseGameObject* pFrom)
{
    switch (pFrom->Type())
    {
        ...
        case AETypes::eSlog_126:
        {
            ...
            BlowUp_436510();
            auto pExplosion = ae_new<Explosion>(mXPos, mYPos - (mSpriteScale * FP_FromInteger(5)), mSpriteScale, 1);
            if (!pExplosion)
            {
                return 1;
            }
...
void FlyingSlig::Brain_4_ChasingEnemy()
{
    ...
    if (Event_Get(kEventHeroDying) && gMap.Is_Point_In_Current_Camera_4810D0(mLvlNumber, mPathNumber, mXPos, mYPos, 0))
    {
        ToAbeDead_436010();
        return;
    }

    if (Event_Get(kEventResetting) || sControlledCharacter->mSpriteScale != mSpriteScale || IsInInvisibleZone(sControlledCharacter) || sControlledCharacter->mAliveGameObjectFlags.Get(Flags_114::e114_Bit8_bInvisible) || (!IsWallBetween_43A550(this, sControlledCharacter) && (sControlledCharacter != sActiveHero || sActiveHero->mCurrentMotion != eAbeMotions::Motion_65_LedgeAscend_4548E0) && sControlledCharacter->Type() != AETypes::eMineCar_89))
    {
        PatrolDelay_435860();
        return;
    }

    if (sub_436C60(&mCollisionLine->field_0_rect, 1))
    {
...
void FlyingSlig::Brain_7_PanicMoving()
{
    ...
    if (!Event_Is_Event_In_Range(kEventAbeOhm, mXPos, mYPos, mScale))
    {
        ToMoving_435720();
        return;
    }
...
void FlyingSlig::Brain_8_PanicIdle()
{
    ...
    else if (Event_Is_Event_In_Range(kEventAbeOhm, mXPos, mYPos, mScale))
    {
...
void FlyingSlig::Brain_14_DePossession()
{
    ...
    if (static_cast<s32>(sGnFrame_5C1B84) <= field_14C_timer)
    {
        ...
        {
            const FP xOff = (mSpriteScale * FP_FromInteger(Math_RandomRange(-20, 20) + (mAnim.mAnimFlags.Get(AnimFlags::eBit5_FlipX) ? -10 : 10)));
...
{
    if (mAnim.field_92_current_frame == 1 && field_17E_flags.Get(Flags_17E::eBit1_Speaking_flag1))
    {
        field_17E_flags.Clear(Flags_17E::eBit1_Speaking_flag1);
...
{
    return ((field_184_xSpeed > FP_FromInteger(0) && !(mAnim.mAnimFlags.Get(AnimFlags::eBit5_FlipX)))
            || (field_184_xSpeed < FP_FromInteger(0) && mAnim.mAnimFlags.Get(AnimFlags::eBit5_FlipX)));
}
...
{
    if (!gMap.Is_Point_In_Current_Camera_4810D0(mLvlNumber, mPathNumber, mXPos, mYPos, 0) || !gMap.Is_Point_In_Current_Camera_4810D0(mLvlNumber, mPathNumber, mXPos, mYPos, 0) || Event_Get(kEventResetting) || IsAbeEnteringDoor_43B030(pObj) || sActiveHero->mSpriteScale != mSpriteScale || !IsWallBetween_43A550(this, pObj))
    {
        return 0;
    }
    ...
    if (VIsFacingMe(pObj) && !IsInInvisibleZone(pObj) && !pObj->mAliveGameObjectFlags.Get(Flags_114::e114_Bit8_bInvisible))
    {
        return 1;
    }
...
Bool32 FlyingSlig::IsWallBetween_43A550(BaseAliveGameObject* pThis, BaseAliveGameObject* pObj)
{
    ...
    return sCollisions_DArray_5C1128->Raycast(
               ...
               &hitX,
               &hitY,
               (pThis->mScale != 0 ? 1 : 16) | (pThis->mScale != 0 ? 6 : 96) | (pThis->mScale != 0 ? 8 : 0x80))
        != 1;
}
...
void FlyingSlig::ThrowGrenade_43A1E0()
{
    ...
    auto pGrenade = ae_new<Grenade>(grenadeXPos + mXPos, grenadeYPos + mYPos, 0, 1, 0, this);
    if (pGrenade)
    {
    ...
    New_ShootingFire_Particle(xpos + mXPos, ypos + mYPos, mAnim.mAnimFlags.Get(AnimFlags::eBit5_FlipX), mSpriteScale);
...
void FlyingSlig::BlowUp_436510()
{
    ...
    ae_new<Blood>(mXPos, mYPos - (FP_FromInteger(30) * mSpriteScale), FP_FromInteger(0), FP_FromInteger(0), mSpriteScale, 20);
...
{
    return sActiveHero == Event_Is_Event_In_Range(kEventSuspiciousNoise, mXPos, mYPos, mScale) || sActiveHero == Event_Is_Event_In_Range(kEventSpeaking, mXPos, mYPos, mScale);
}
...
s16 FlyingSlig::sub_437C70(PathLine* pLine)
{
    ...
    field_1A4_rect.x = FP_FromInteger(mCollisionLine->field_0_rect.x);
    field_1A4_rect.y = FP_FromInteger(mCollisionLine->field_0_rect.y);
    field_1A4_rect.w = FP_FromInteger(mCollisionLine->field_0_rect.w);
    field_1A4_rect.h = FP_FromInteger(mCollisionLine->field_0_rect.h);


    field_1EC_pNextLine = sCollisions_DArray_5C1128->Get_Line_At_Idx(mCollisionLine->field_C_next);
    field_1F0_pPrevLine = sCollisions_DArray_5C1128->Get_Line_At_Idx(mCollisionLine->field_A_previous);

    field_198_line_length = FP_FromInteger(mCollisionLine->field_12_line_length);
...
Bool32 FlyingSlig::sub_436B20()
{
    ...
    PathLine* pNextLine = sCollisions_DArray_5C1128->Get_Line_At_Idx(mCollisionLine->field_C_next);
    if (pNextLine)
    {
        while (pNextLine != mCollisionLine)
        {
    ...
    PathLine* pPrevLine = sCollisions_DArray_5C1128->Get_Line_At_Idx(mCollisionLine->field_A_previous);
    if (pPrevLine)
    {
        while (pPrevLine != mCollisionLine)
        {
...
s16 FlyingSlig::CollisionUp_43A640(FP velY)
{
    ...
    auto bCollision = sCollisions_DArray_5C1128->Raycast(
        ...
        &hitX,
        &hitY,
        (mScale != 0 ? 0x20000 : 0x40000) | (mScale != 0 ? 1 : 0x10) | (mScale != 0 ? 8 : 0x80));
    ...
    if (!bCollision)
    {
        bCollision = sCollisions_DArray_5C1128->Raycast(
            ...
            &hitX,
            &hitY,
            (mScale != 0 ? 0x20000 : 0x40000) | (mScale != 0 ? 1 : 16) | (mScale != 0 ? 8 : 128));
    }
...
s16 FlyingSlig::CollisionDown_43A9E0(FP velY)
{
    ...
    auto bCollision = sCollisions_DArray_5C1128->Raycast(
        ...
        &hitX,
        &hitY,
        (mScale != 0 ? 0x20000 : 0x40000) | (mScale != 0 ? 1 : 0x10));
...
s16 FlyingSlig::CollisionLeftRight_43AC80(FP velX)
{
    ...
    auto bCollision = sCollisions_DArray_5C1128->Raycast(
        ...
        &hitX,
        &hitY,
        (mScale != 0 ? 0x20000 : 0x40000) | (mScale != 0 ? 6 : 96));
    ...
    if (bCollision)
    {
        if (static_cast<s32>(sGnFrame_5C1B84) > field_154_collision_reaction_timer)
        {
            Slig_GameSpeak_SFX_4C04F0(sGnFrame_5C1B84 & 1 ? SligSpeak::eOuch2_14 : SligSpeak::eOuch1_13, 127, Math_RandomRange(256, 512), this);
            field_154_collision_reaction_timer = (Math_NextRandom() & 3) + sGnFrame_5C1B84 + 10;
            ae_new<ParticleBurst>(sparkX, hitY + (FP_FromInteger(16) * mSpriteScale), 5u, mSpriteScale, BurstType::eSmallPurpleSparks_6, 9);
        }
...
void FlyingSlig::sub_437AC0(FP arg1, FP_Point* pPoint)
{
    ...
    {
        pPoint->field_0_x = mXPos;
        pPoint->field_4_y = mYPos;
        return;
    }
