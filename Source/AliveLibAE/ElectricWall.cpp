...
{
    if (gMap.mCurrentLevel != gMap.mLevel || gMap.mCurrentPath != gMap.mPath || gMap.GetDirection_4811A0(mLvlNumber, mPathNumber, mXPos, mYPos) == CameraPos::eCamInvalid_m1)
    {
                ...
                default:
                    if (pObj->mScale == mScale)
                    {
                        ...
                        else
                        {
                            ...
                            if (!(pObj->mAliveGameObjectFlags.Get(Flags_114::e114_Bit7_Electrocuted)) && (pObj != sActiveHero || !gAbeBulletProof_5C1BDA))
                            {
                                pObj->mAliveGameObjectFlags.Set(Flags_114::e114_Bit7_Electrocuted);
