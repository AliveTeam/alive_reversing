...
{
    for (; tlvIterator.GetTlv(); tlvIterator = gPathInfo->TLV_Get_At(
                     tlvIterator,
                     mXPos,
                     mYPos,
                     mXPos,
                     mYPos))
    {
        ...
        {
            auto pContinuePoint = static_cast<relive::Path_ContinuePoint*>(tlvIterator.GetTlv());
        ...
        else if (tlvIterator.GetTlv()->mTlvType == ReliveTypes::eResetPath)
        {
            auto pResetSwitchRange = static_cast<relive::Path_ResetPath*>(tlvIterator.GetTlv());
...
void Abe::Motion_0_Idle_44EEB0()
{
    ...
    if (Input().IsAnyHeld(InputCommands::eUp))
    {
        ...
        // Get the first TLV
        TlvIterator tlvIterator = gPathInfo->TLV_Get_At(
            TlvIterator::Invalid(),
            mXPos,
            mYPos,
            mXPos,
        ...
        while (tlvIterator.GetTlv())
        {
            switch (tlvIterator.GetTlv()->mTlvType)
            {
                ...
                case ReliveTypes::eWellLocal:
                {
                    ...
                    relive::Path_WellLocal* pWell = static_cast<relive::Path_WellLocal*>(tlvIterator.GetTlv());
                ...
                case ReliveTypes::eWellExpress:
                {
                    ...
                    // Bail if scale doesn't match
                    relive::Path_WellBase* pWell = static_cast<relive::Path_WellBase*>(tlvIterator.GetTlv());
                    if ((pWell->mScale != relive::reliveScale::eFull || GetSpriteScale() != FP_FromDouble(1.0)) && (pWell->mScale != relive::reliveScale::eHalf || GetSpriteScale() != FP_FromDouble(0.5)))
                    {
                        break;
                    }
...
void Abe::Motion_3_Fall_459B60()
{
    ...
    if (BaseAliveGameObjectPathTLV.GetTlv())
    {
        if (mHealth > FP_FromInteger(0))
        {
            if (BaseAliveGameObjectPathTLV.GetTlv()->mTlvType == ReliveTypes::eWellLocal || BaseAliveGameObjectPathTLV.GetTlv()->mTlvType == ReliveTypes::eWellExpress)
            {
                ...
                relive::Path_WellBase* pWellBase = static_cast<relive::Path_WellBase*>(BaseAliveGameObjectPathTLV.GetTlv());
    ...
    TlvIterator edgeIterator = gPathInfo->VTLV_Get_At_Of_Type(
                    FP_GetExponent(mXPos),
                    FP_GetExponent(mYPos - (GetSpriteScale() * FP_FromInteger(75))),
                    FP_GetExponent(mXPos),
                    FP_GetExponent(mYPos),
                    ReliveTypes::eEdge);
    relive::Path_Edge* pEdge = static_cast<relive::Path_Edge*>(edgeIterator.GetTlv());
    ...
    else // Didn't find and edge to grab so check if falling onto a hoist
    {
        ...
        TlvIterator hoistIterator = gPathInfo->VTLV_Get_At_Of_Type(
            FP_GetExponent(mXPos),
            FP_GetExponent(mYPos - GetSpriteScale() * FP_FromInteger(20)),
            FP_GetExponent(mXPos),
            FP_GetExponent(mYPos - GetSpriteScale() * FP_FromInteger(20)),
            ReliveTypes::eHoist);
        relive::Path_Hoist* pHoist = static_cast<relive::Path_Hoist*>(hoistIterator.GetTlv());
    ...
    if (tryToHang)
    {
        ...
        mXPos = FP_FromInteger((BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftX + BaseAliveGameObjectPathTLV.GetTlv()->mBottomRightX) / 2);

        MapFollowMe(true);

        if (!gCollisions->Raycast(
                mXPos,
                FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY - 10), // TODO: Negative ??
                mXPos,
                FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY + 10),
                &pPathLine,
                &hitX,
                &hitY,
        ...
        mVelY = FP_FromInteger(0);
        mVelX = FP_FromInteger(0);
        if (BaseAliveGameObjectPathTLV.GetTlv()->mTlvType != ReliveTypes::eHoist || (FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mBottomRightY - 1 * BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY)) >= (GetSpriteScale() * FP_FromInteger(70)))
        {
...
void Abe::Motion_14_HoistIdle_452440()
{
    ...
    if (mVelY < FP_FromInteger(0))
    {
        ...
        if (FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mBottomRightY + -BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY) > (GetSpriteScale() * FP_FromInteger(90)) || GetAnimation().GetCurrentFrame())
        {
            return;
        }
    }
    ...
    if (pHoist)
    {
        if (IsSameScaleAsHoist(pHoist, this) && (IsFacingSameDirectionAsHoist(pHoist, this) || pHoist->mGrabDirection == relive::Path_Hoist::GrabDirection::eFacingAnyDirection))
        {
            if (pHoist->mHoistType == relive::Path_Hoist::Type::eOffScreen)
            {
                ...
                if (FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mBottomRightY - 1 * BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY)
            ...
            {
                Environment_SFX(EnvironmentSfx::eWalkingFootstep_1, 0, 127, this);
                if (FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mBottomRightY - 1 * BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY) >= GetSpriteScale() * FP_FromInteger(70))
                {
                    mCurrentMotion = eAbeMotions::Motion_67_LedgeHang_454E20;
                }
                else
...
void Abe::Motion_28_HopMid_451C50()
{
    ...
    else
    {
        ...
        // Check for an edge
        TlvIterator tlvIterator = gPathInfo->VTLV_Get_At_Of_Type(
            FP_GetExponent(mXPos),
            FP_GetExponent(mYPos - (GetSpriteScale() * FP_FromInteger(75))),
            FP_GetExponent(mXPos),
            FP_GetExponent(mYPos),
        ...
        relive::Path_Edge* pEdgeTlv = static_cast<relive::Path_Edge*>(tlvIterator.GetTlv());
    ...
    else
    {
        ...
        else
        {
            TlvIterator edgeIterator = gPathInfo->VTLV_Get_At_Of_Type(
                ...
                FP_GetExponent(mYPos),
                ReliveTypes::eEdge);
            relive::Path_Edge* pEdgeTlv = static_cast<relive::Path_Edge*>(edgeIterator.GetTlv());
            BaseAliveGameObjectPathTLV = edgeIterator;
...
void Abe::Motion_68_ToOffScreenHoist_454B80()
{
    ...
    // Get the connecting to hoist (on the above screen - can only hoist up)
    const FP ypos = FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY) - (FP_FromInteger(40) * GetSpriteScale());
    TlvIterator hoistIterator = gPathInfo->VTLV_Get_At_Of_Type(
        FP_GetExponent(mXPos),
        FP_GetExponent(ypos),
        FP_GetExponent(mXPos),
    ...
    FP hitX = {};
    FP hitY = {};
    if (hoistIterator.GetTlv() && gCollisions->Raycast(mXPos, FP_FromInteger(hoistIterator.GetTlv()->mTopLeftY - 10), mXPos, FP_FromInteger(hoistIterator.GetTlv()->mTopLeftY + 10), &pLine, &hitX, &hitY, GetScale() == Scale::Fg ? kFgFloor : kBgFloor))
    {
        BaseAliveGameObjectCollisionLine = pLine;
        mYPos = FP_NoFractional(hitY + FP_FromDouble(0.5));
        mVelY = FP_FromInteger(0);
...
void Abe::Motion_78_WellBegin_45C810()
{
    if (GetAnimation().GetCurrentFrame() > 10)
    {
        ...
        const s16 tlv_mid_x = (BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftX + BaseAliveGameObjectPathTLV.GetTlv()->mBottomRightX) / 2;
        if (xpos > tlv_mid_x)
        {
    ...
    if (GetAnimation().GetIsLastFrame())
    {
        ...
        if (gPathInfo->VTLV_Get_At_Of_Type(
                ...
                FP_GetExponent(mXPos),
                FP_GetExponent(mYPos),
                ReliveTypes::eWellExpress).GetTlv())
        {
...
void Abe::Motion_79_InsideWellLocal_45CA60()
{
    ...
    if (!gnFrame)
    {
        ...
        relive::Path_WellBase* pBaseWell = static_cast<relive::Path_WellBase*>(BaseAliveGameObjectPathTLV.GetTlv());
...
void Abe::Motion_83_WellExpressShotOut_45CF70()
{
    ...
    TlvIterator tlvIter = gPathInfo->Get_First_TLV_For_Offsetted_Camera(0, 0);
    while (tlvIter.GetTlv())
    {
        // Is it a well?
        if (tlvIter.GetTlv()->mTlvType == ReliveTypes::eWellLocal || tlvIter.GetTlv()->mTlvType == ReliveTypes::eWellExpress)
        {
            // Is it the target of the previous well?
            auto pWellBase = tlvIter.GetTlv<relive::Path_WellBase>();
    ...
    if (pWell)
    {
        ...
        BaseAliveGameObjectPathTLV = tlvIter;
    ...
    switch (field_120_state.stone)
    {
        case StoneStates::eHandstoneBegin_0:
            if (GetAnimation().GetForwardLoopCompleted())
            {
                ...
                s32 switch_id = 0;
                relive::Path_MovieStone* pMovieStoneTlv = static_cast<relive::Path_MovieStone*>(BaseAliveGameObjectPathTLV.GetTlv());
                if (!pMovieStoneTlv)
                {
                    TlvIterator handStoneIterator = gPathInfo->VTLV_Get_At_Of_Type(
                        FP_GetExponent(mXPos),
                        FP_GetExponent(mYPos),
                        FP_GetExponent(mXPos),
                        FP_GetExponent(mYPos),
                        ReliveTypes::eHandStone);
                    BaseAliveGameObjectPathTLV = handStoneIterator;
                    relive::Path_HandStone* pHandStoneTlv = handStoneIterator.GetTlv<relive::Path_HandStone>();
...
void Abe::Motion_114_DoorEnter()
{
    switch (field_120_state.door)
    {
        ...
        case AbeDoorStates::eSetNewActiveCamera_4:
        {
            ...
            auto pDoorTlv = BaseAliveGameObjectPathTLV.GetTlv<relive::Path_Door>();
        ...
        case AbeDoorStates::eSetNewAbePosition_5:
        {
            ...
            {
                doorTlvIterator = Path::TLV_Next_Of_Type(doorTlvIterator, ReliveTypes::eDoor);
            }
            
            auto pTargetDoorTlv = doorTlvIterator.GetTlv<relive::Path_Door>();
            BaseAliveGameObjectPathTLV = doorTlvIterator;
            if (!pTargetDoorTlv || pTargetDoorTlv->mDoorId != field_1A0_door_id)
            {
            ...
            mXPos = FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftX) + FP_FromInteger((BaseAliveGameObjectPathTLV.GetTlv()->mBottomRightX - BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftX) / 2);
            ...
            else
            {
                ...
                BaseAliveGameObjectLastLineYPos = FP_FromInteger(BaseAliveGameObjectPathTLV.GetTlv()->mTopLeftY);
            }
...
void Abe::Motion_127_TurnWheelLoop()
{
    ...
    {
        relive::Path_LevelLoader* pLevelLoader = static_cast<relive::Path_LevelLoader*>(gPathInfo->TLV_First_Of_Type_In_Camera(ReliveTypes::eLevelLoader, 0).GetTlv());
    ...
    else if (field_120_state.wheel == WorkWheelStates::eMapChanging_2)
    {
                ...
                0))
        {
            ...
            auto pPathAbeStart = static_cast<relive::Path_AbeStart*>(gPathInfo->TLV_First_Of_Type_In_Camera(ReliveTypes::eAbeStart, 0).GetTlv());
    ...
    if (Input().IsAnyHeld(InputCommands::eUp))
    {
        for (s32 idx = 0; idx < gBaseAliveGameObjects->Size(); idx++)
        {
            ...
            if (pObj->GetAnimation().GetRender())
            {
                ...
                if (abeRect.Overlaps(mineCarRect) && pObj->GetSpriteScale() == GetSpriteScale() && pObj->Type() == ReliveTypes::eMineCar)
                {
...
s16 Abe::RunTryEnterDoor_451220()
{
    ...
    TlvIterator doorIterator = gPathInfo->VTLV_Get_At_Of_Type(
        FP_GetExponent(mXPos),
        FP_GetExponent(mYPos),
        FP_GetExponent(mXPos),
        FP_GetExponent(mYPos),
    ...
    field_120_state.raw = 0;
    mCurrentMotion = eAbeMotions::Motion_114_DoorEnter;
    mXPos = FP_FromInteger((doorIterator.GetTlv()->mTopLeftX + doorIterator.GetTlv()->mBottomRightX) / 2);
    MapFollowMe(true);
    return 1;
}
...
s16 Abe::RunTryEnterWell_451060()
{
    ...
    if (tlvIterator.GetTlv())
    {
        if (!(GetTeleporting()))
        {
            auto pWellLocal = static_cast<relive::Path_WellLocal*>(tlvIterator.GetTlv());
    ...
    if (tlvIterator.GetTlv())
    {
        if (!(GetTeleporting()))
        {
            auto pWellExpress = static_cast<relive::Path_WellExpress*>(tlvIterator.GetTlv());
...
s16 Abe::GetEvilFart_4585F0(s16 bDontLoad)
{
    ...
    for (s32 i = 0; i < gBaseGameObjects->Size(); i++)
    {
        ...
        if (pObj->Type() == ReliveTypes::eBrewMachine)
        {
            ...
            if (abeRect.Overlaps(bRect) && pBrewMachine->GetSpriteScale() == GetSpriteScale() && pBrewMachine->GetRemainingBrewCount() > 0 && !mHasEvilFart)
            {
                break;
            }
