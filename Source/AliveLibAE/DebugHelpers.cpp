    ...
    void Init()
    {
        ...
        for (s32 i = 0; i < 5; i++)
        {
            if (i < 2)
            {
                ...
                mAnim[i].Init(loadingRec.mFrameTableOffset, gAnimations, this, loadingRec.mMaxW, loadingRec.mMaxH, mAnimRes[i], 1);
            }
            else
            {
                // 8 bit
                mAnimRes[i] = ResourceManager::GetLoadedResource(ResourceManager::Resource_Animation, abeGibRec.mResourceId, TRUE, FALSE);
                mAnim[i].Init(abeGibRec.mFrameTableOffset, gAnimations, this, abeGibRec.mMaxW, abeGibRec.mMaxH, mAnimRes[i], 1);
            }
