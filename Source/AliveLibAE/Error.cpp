...
/*EXPORT not big enough to hook*/ void CC Error_NullPrint_4F28D0(const s8* msg)
{
    LOG_ERROR(msg);
}
...
{
    static s8 sErrorMessage_BBFBA8[2052];
    static s8 sErrorTitle_BC03B4[2048];
...
{
    static s8 sErrorMsg_BBEEFC[2048];
    static s8 sErrorTitle_BBF6FC[1024];
...
{
    static s8 sWarningMsg_BBE6FC[2048];
    va_list va;
...
EXPORT void CC Error_ShowErrorStackToUser_4F2A70(bool bDisplayAll)
{
    ...
    if (bDisplayAll)
    {
        ...
        s8* allocatedString = reinterpret_cast<s8*>(ae_malloc_4F4E60(sErrorIndex_BBC564 * 256));
        if (allocatedString)
        {
            allocatedString[0] = 0;
