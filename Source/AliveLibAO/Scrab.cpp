...
Scrab::Scrab(Path_Scrab* pTlv, s32 tlvInfo)
    : BaseAliveGameObject()
{
    mBaseGameObjectTypeId = Types::eScrab_77;
...
Scrab* Scrab::FindScrabToFight()
{
    for (s32 i = 0; i < gBaseGameObjects->Size(); i++)
    {
        ...
        if (pObj->mBaseGameObjectTypeId == Types::eScrab_77)
        {
...
{
    if (CanSeeAbe(sActiveHero_507678) && sActiveHero_507678->mHealth > FP_FromInteger(0) && sActiveHero_507678->field_BC_sprite_scale == field_BC_sprite_scale && !WallHit_401930(sActiveHero_507678->field_A8_xpos - field_A8_xpos, field_BC_sprite_scale * FP_FromInteger(35)))
    {
    ...
    for (s32 i = 0; i < gBaseGameObjects->Size(); i++)
    {
        ...
        if (pObjIter->mBaseGameObjectFlags.Get(BaseGameObject::eIsBaseAliveGameObject_Bit6))
        {
            ...
            if (pObj->mBaseGameObjectTypeId == Types::eMudokon_52 || pObj->mBaseGameObjectTypeId == Types::eMudokon_75 || pObj->mBaseGameObjectTypeId == Types::SlingMud_90)
            {
...
s16 Scrab::Brain_ChasingEnemy_45CC90()
{
    ...
    switch (field_110_brain_sub_state)
    {
        ...
        case 3:
        {
            ...
            auto pLiftPoint = static_cast<LiftPoint*>(mLiftPoint);
            if (pLiftPoint && mLiftPoint->mBaseGameObjectTypeId == Types::eLiftPoint_51 && !pLiftPoint->OnAnyFloor())
            {
                mNextMotion = eScrabMotions::Motion_1_Stand_45E620;
                return 4;
            }
...
s16 Scrab::Brain_Patrol_460020()
{
    ...
    switch (field_110_brain_sub_state)
    {
        case 0:
        {
            ...
            if (pLiftPoint)
            {
                if (pLiftPoint->mBaseGameObjectTypeId == Types::eLiftPoint_51)
                {
                    if (!pLiftPoint->OnAnyFloor())
                    {
                        SetBrain(&Scrab::Brain_WalkAround_460D80);
        ...
        case 1:
            ...
            {
                auto pLiftPoint = static_cast<LiftPoint*>(mLiftPoint);
                if (pLiftPoint->mBaseGameObjectTypeId == Types::eLiftPoint_51 && !pLiftPoint->OnAnyFloor())
                {
                    mNextMotion = eScrabMotions::Motion_1_Stand_45E620;
                    return 8;
                }
            ...
            if (gMap.TLV_Get_At_446260(
                    FP_GetExponent(field_A8_xpos - ScaleToGridSize(field_BC_sprite_scale)),
                    FP_GetExponent(field_AC_ypos),
                    FP_GetExponent(field_A8_xpos - ScaleToGridSize(field_BC_sprite_scale)),
                    FP_GetExponent(field_AC_ypos),
                    TlvTypes::ScrabLeftBound_74))
        ...
        case 4:
            ...
            if (gMap.TLV_Get_At_446260(
                    FP_GetExponent(ScaleToGridSize(field_BC_sprite_scale) + field_A8_xpos),
                    FP_GetExponent(field_AC_ypos),
                    FP_GetExponent(ScaleToGridSize(field_BC_sprite_scale) + field_A8_xpos),
                    FP_GetExponent(field_AC_ypos),
                    TlvTypes::ScrabRightBound_75))
