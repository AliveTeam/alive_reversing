...
void Kill_Objects_451720()
{
    ...
    for (s32 i = 0; i < 2; i++)
    {
        for (s32 j = 0; j < gBaseGameObjects->Size(); j++)
        {
            ...
            if (!pObj->mGameObjectFlags.Get(BaseGameObject::eSurviveDeathReset_Bit9) && pObj->field_C_refCount == 0)
            {
                // So die
...
void SaveGame::LoadFromMemory(SaveData* pData, s32 bKillObjects)
{
    ...
    sActiveHero->field_144_saved_level = pData->field_212_saved_level;
    ...
    sActiveHero->field_11C_regen_health_timer = gnFrameCount_507670;
    ...
    sActiveHero->mAnim.mAnimFlags.Set(AnimFlags::eBit5_FlipX, pData->field_23C_ah_flipX & 1);

    sActiveHero->mAnim.mAnimFlags.Clear(AnimFlags::eBit3_Render);
...
void SaveGame::SaveToMemory(SaveData* pSaveData)
{
    ...
    pSaveData->field_212_saved_level = sActiveHero->field_144_saved_level;
    ...
    pSaveData->field_240_last_anim_frame = sActiveHero->mAnim.field_92_current_frame;
    ...
    {
        pSaveData->field_23A_mode_mask = sActiveHero->mCollisionLine->field_8_type;
    }
    else
    {
    ...
    pSaveData->field_23C_ah_flipX = sActiveHero->mAnim.mAnimFlags.Get(AnimFlags::eBit5_FlipX);
    ...
    if (gElum_507680 != 0)
    {
        pSaveData->field_25C_bControllingElum = sControlledCharacter == gElum_507680;
        ...
        pSaveData->field_260_elum_lvl_number = gElum_507680->mLvlNumber;
        pSaveData->field_262_elum_path_number = gElum_507680->mPathNumber;
        ...
        if (gElum_507680->mCollisionLine)
        {
            pSaveData->field_270_elum_line_type = gElum_507680->mCollisionLine->field_8_type;
        }
        else
        {
        ...
        pSaveData->field_272_elum_flipX = gElum_507680->mAnim.mAnimFlags.Get(AnimFlags::eBit5_FlipX);
