...
void ParamiteWeb::VRender(PrimHeader** ppOt)
{
    ...
    if (mLvlNumber == gMap.mCurrentLevel && mPathNumber == gMap.mCurrentPath)
    {
        if (mXPos >= FP_FromInteger(camCoords.field_0_x) && mXPos <= FP_FromInteger(camCoords.field_0_x + 1024))
        {
            ...
            const s16 x_start = PsxToPCX<s16>(FP_GetExponent(mXPos + FP_FromInteger(pScreenManager_4FF7C8->field_14_xpos) - cam_x));
            ...
            if (y_start >= minY)
            {
                for (s32 idx = 0; idx < field_E4_number_of_segments; ++idx)
                {
                    ...
                    field_EC_pRes[idx].VRender2(x_start, y_start + mYOffset, ppOt);
                    PSX_RECT rect = {};
                    ...
                    ClipPoly_Vertically_4584B0(&field_EC_pRes[idx].field_10_polys[gPsxDisplay_504C78.field_A_buffer_index], mYOffset + minY, mYOffset + maxY);
