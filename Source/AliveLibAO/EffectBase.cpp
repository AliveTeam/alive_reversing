    ...
    : BaseGameObject(TRUE)
{
    ...
    for (s32 i = 0; i < 2; i++)
    {
        Init_SetTPage_495FB0(&mTPage[i], 0, 0, static_cast<s16>(PSX_getTPage_4965D0(TPageMode::e16Bit_2, abr, 0, 0)));
    }
    mLayer = layer;
    mSemiTrans = 1;
}
...
{
    Prim_Tile* pTile = &mTile[gPsxDisplay_504C78.field_A_buffer_index];
    Init_Tile(pTile);
    ...
    OrderingTable_Add_498A80(OtLayer(ppOt, mLayer), &pTile->mBase.header);
    OrderingTable_Add_498A80(OtLayer(ppOt, mLayer), &mTPage[gPsxDisplay_504C78.field_A_buffer_index].mBase);
