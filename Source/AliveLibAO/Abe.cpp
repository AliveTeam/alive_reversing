...
void Abe::VUpdate()
{
    if (!field_2AA_flags.Get(Flags_2AA::e2AA_Bit1))
    {
        ...
        else
        {
            ...
            {
                // Did position change?
                if (oldX != mXPos || oldY != mYPos || sControlledCharacter_50767C == gElum_507680)
                {
...
{
    if (!Input().IsAnyPressed(sInputKey_Up_4C6598) || mAnim.mCurrentFrame < 4)
    {
        return 0;
    }
    ...
    if (mCurrentMotion != eAbeMotions::Motion_137_ElumUnmountBegin_42E2B0 && mCurrentMotion != eAbeMotions::Motion_138_ElumUnmountEnd_42E390)
    {
        switch (elumMotion)
        {
            case eElumMotions::Motion_1_Idle_412990:
                ...
                ToNewElumSyncMotion_422520(gElum_507680->mAnim.mCurrentFrame);
...
void Abe::ToNewElumSyncMotion_422520(s16 elum_frame)
{
    ...
    mAnim.mFlags.Set(AnimFlags::eBit5_FlipX, gElum_507680->mAnim.mFlags.Get(AnimFlags::eBit5_FlipX));
}
...
s16 Abe::TryMountElum_42E600()
{
    if (gElum_507680)
    {
        ...
        {
            if (gElum_507680->mAnim.mFlags.Get(AnimFlags::eBit5_FlipX) != mAnim.mFlags.Get(AnimFlags::eBit5_FlipX))
            {
...
s16 Abe::RunTryEnterDoor_4259C0()
{
    ...
    mXPos = FP_FromInteger((pDoorTlv->mBottomRight.x + pDoorTlv->mTopLeft.x) / 2);
...
s16 Abe::MoveLiftUpOrDown_42F190(FP yVelocity)
{
    ...
    if (sControlledCharacter_50767C == this && !mAnim.mFlags.Get(AnimFlags::eBit18_IsLastFrame) && mAnim.mCurrentFrame != 5)
    {
    ...
    while (pTlv)
    {
        switch (pTlv->mTlvType32.mType)
        {
            ...
            case TlvTypes::Lever_26:
                if (FP_FromInteger(FP_GetExponent(mXPos) - pTlv->mTopLeft.x) < ScaleToGridSize(mSpriteScale))
                {
                    // Wrong dir
                    if (mAnim.mFlags.Get(AnimFlags::eBit5_FlipX))
                    {
                ...
                else if (FP_FromInteger(pTlv->mBottomRight.x - FP_GetExponent(mXPos)) < ScaleToGridSize(mSpriteScale))
                {
                    // Wrong dir
                    if (!mAnim.mFlags.Get(AnimFlags::eBit5_FlipX))
                    {
...
{
    if (pHoist->field_1A_grab_direction == Path_Hoist::GrabDirection::eFacingLeft && !pObj->mAnim.mFlags.Get(AnimFlags::eBit5_FlipX))
    {
        return false;
    }
    else if (pHoist->field_1A_grab_direction == Path_Hoist::GrabDirection::eFacingRight && pObj->mAnim.mFlags.Get(AnimFlags::eBit5_FlipX))
    {
        return false;
    }
    return true;
...
void Abe::TryHoist_423420()
{
    ...
    {
        if (FP_FromInteger(pHoist->mBottomRight.y - pHoist->mTopLeft.y) <= mSpriteScale * FP_FromInteger(95))
        {
            mCurrentMotion = eAbeMotions::Motion_16_HoistBegin_426E40;
        }
        else
...
{
    if (pEdge->field_18_grab_direction == Path_Edge::GrabDirection::eFacingLeft && pObj->mAnim.mFlags.Get(AnimFlags::eBit5_FlipX))
    {
        return true;
    }
    else if (pEdge->field_18_grab_direction == Path_Edge::GrabDirection::eFacingRight && !pObj->mAnim.mFlags.Get(AnimFlags::eBit5_FlipX))
    {
        return true;
    }
...
void Abe::Motion_3_Fall_42E7F0()
{
    ...
    if (tryToHang)
    {
        ...
        mXPos = FP_FromInteger((BaseAliveGameObjectPathTLV->mTopLeft.x + BaseAliveGameObjectPathTLV->mBottomRight.x) / 2);
...
void Abe::Motion_27_RunSlideStop_425B60()
{
    ...
    else
    {
            ...
            && !RunTryEnterDoor_4259C0())
        {
            ...
            else if ((mAnim.mFlags.Get(AnimFlags::eBit5_FlipX) && Input().IsAnyPressed(sInputKey_Right_4C6590)) || (!mAnim.mFlags.Get(AnimFlags::eBit5_FlipX) && Input().IsAnyPressed(sInputKey_Left_4C6594)))
            {
...
void Abe::Motion_31_HopLand_426940()
{
    ...
    if (mAnim.mCurrentFrame == 2 && Input().IsAnyPressed(sInputKey_Hop_4C65A0))
    {
        field_2A8_flags.Set(Flags_2A8::e2A8_Bit2_return_to_previous_motion);
...
void Abe::Motion_37_DunnoMid_4232C0()
{
    ...
    if (!Input().IsAnyPressed(sInputKey_DoAction_4C65A4 | sInputKey_ThrowItem_4C65B4) || mAnim.mFlags.Get(AnimFlags::eBit18_IsLastFrame))
    {
        mCurrentMotion = eAbeMotions::Motion_38_DunnoEnd_423310;
    }
}
...
void Abe::Motion_61_Respawn_42CD20()
{
    ...
    switch (field_114_gnFrame)
    {
        ...
        case 1:
        {
            mCurrentLevel = field_144_saved_level;
            mCurrentPath = field_142_saved_path;
            mXPos = FP_FromInteger(field_138_zone_top_left.x);
            mYPos = FP_FromInteger(field_138_zone_top_left.y) + mSpriteScale * FP_FromInteger(30);

            BaseAliveGameObjectLastLineYPos = mYPos;

            mAnim.mFlags.Set(AnimFlags::eBit5_FlipX, field_2A8_flags.Get(Flags_2A8::e2A8_eBit16_AbeSpawnDir));
            MapFollowMe_401D30(TRUE);
            SetTint(sAbeTints_4C6438, gMap.mCurrentLevel);
            if (gElum_507680)
            {
...
void Abe::Motion_62_LoadedSaveSpawn_45ADD0()
{
    ...
    if (field_114_gnFrame)
    {
        ...
        if (sCollisions->Raycast(
                ...
                sActiveHero_507678->mYPos - FP_FromInteger(60),
                sActiveHero_507678->mXPos,
                sActiveHero_507678->mYPos + FP_FromInteger(60),
                &pLine2,
                &hitX2,
                ...
                CollisionMask(static_cast<eLineTypes>(pSaveData->field_23A_mode_mask))))
        {
            ...
            sActiveHero_507678->mYPos = hitY2;
        ...
        sActiveHero_507678->BaseAliveGameObjectLastLineYPos = sActiveHero_507678->mYPos;
        ...
        sActiveHero_507678->mAnim.mFlags.Set(AnimFlags::eBit5_FlipX, pSaveData->field_23C_ah_flipX & 1);
        sActiveHero_507678->MapFollowMe_401D30(TRUE);
        sActiveHero_507678->mAnim.mFlags.Set(AnimFlags::eBit3_Render);
        ...
        if (pSaveData->field_25A_bElumExists)
        {
            ...
            gElum_507680->mAnim.mFlags.Clear(AnimFlags::eBit3_Render);
            ...
            gElum_507680->mCurrentLevel = MapWrapper::FromAO(pSaveData->field_260_elum_lvl_number);
            gElum_507680->mCurrentPath = pSaveData->field_262_elum_path_number;
            ...
            gElum_507680->mAnim.mFlags.Set(AnimFlags::eBit5_FlipX, pSaveData->field_272_elum_flipX & 1);
            ...
            if (gElum_507680->mCurrentPath == sActiveHero_507678->mCurrentPath)
            {
            ...
            gElum_507680->mAnim.mFlags.Set(AnimFlags::eBit3_Render);
        }
    }
    else
    {
...
void Abe::Motion_82_WellExpressShotOut_4315A0()
{
    ...
    if (pWell)
    {
        ...
        mXPos = FP_FromInteger((pWell->mTopLeft.x + pWell->mBottomRight.x) / 2);
        mYPos = FP_FromInteger(pWell->mBottomRight.y);
        field_2A8_flags.Set(Flags_2A8::e2A8_Bit4_snap_abe);
    }
    else
    {
...
{
    if (sActiveHero_507678->mAnim.mCurrentFrame >= 6)
    {
        if (field_15C_pThrowable)
        {
            field_15C_pThrowable->mBaseGameObjectRefCount--;
...
void Abe::Motion_149_PickupItem_42A030()
{
    ...
    if (sActiveHero_507678->mAnim.mCurrentFrame >= 5)
    {
        if (field_15C_pThrowable)
        {
            field_15C_pThrowable->mBaseGameObjectRefCount--;
...
void Abe::Motion_156_DoorEnter_42D370()
{
    switch (field_110_state.door)
    {
        ...
        case AbeDoorStates::eSetNewAbePosition_5:
        {
            ...
            mAnim.mFlags.Set(AnimFlags::eBit5_FlipX, pPathDoor->field_46_abe_direction == XDirection_short::eRight_1);
